...
# for attributes created by +VideoFileColumn+'s +video_file_column+ method. It will be
# automatically included into ActionView::Base, thereby making this module's
# methods available in all your views.
module VideoFileColumnHelper
  
  # Use this helper to create an upload field for a video_file_column attribute. This will generate
  # an additional hidden field to keep uploaded files during form-redisplays. For example,
  # when called with
  #
  #   <%= video_file_column_field("entry", "image") %>
  #
  ...
  def video_file_column_field(object, method, options={})
  ...
  #
  #   <%= url_for_video_file_column("entry", "image")
  #
  ...
  # access different versions of a file as produced by VideoFileColumn::Magick, for example.
  #
  ...
  def url_for_video_file_column(object_name, method, suffix=nil)
