module VideoFileColumn # :nodoc:
  ...
  # If you are using video_file_column to upload images, you can
  ...
  # after a new file is assigned to the video_file_column attribute.
  #
  # To resize the uploaded image according to an imagemagick geometry
  # string, just use the <tt>:geometry</tt> option:
  #
  #    video_file_column :image, :magick => {:geometry => "800x600>"}
  #
  # You can also create additional versions of your image, for example
  # thumb-nails, like this:
  #    video_file_column :image, :magick => {:versions => 
  #      { "thumb" => "50x50", "medium" => "640x480>" }
  #    }
  #
  # These versions can later be accessed via video_file_column's <em>suffix</em>
  ...
  #
  # The same mechanism can be used in the +url_for_video_file_column+ helper:
  #
  #    <%= url_for_video_file_column "entry", "image", "thumb" %>
  #
  # <b>Note:</b> You'll need the
  # rmagick extension installed as a gem in order to use video_file_column's
  # rmagick integration.
  module Magick

    def self.video_file_column(klass, attr, options) # :nodoc:
      require 'RMagick'
      if options[:magick][:versions]
