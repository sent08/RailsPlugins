require 'fileutils'
require 'tempfile'
require 'magick_video_file_column'

module VideoFileColumn # :nodoc:
      ...
      if file.nil?
        ...
        if file.size == 0
          ...
          if file.is_a?(String)
            ...
            raise TypeError.new("Do not know how to handle a string with value '#{file}' that was passed to a video_file_column. Check if the form's encoding has been set to 'multipart/form-data'.")
          end
          upload(file)
        end
      end
  ...
  # == Generated Methods
  #
  # After calling "<tt>video_file_column :image</tt>" as in the example above, a number of instance methods
  ...
  # with a value of +nil+ to +video_file_column+.
  #
  ...
  # by passing in a <tt>:mime_extensions</tt> option to +video_file_column+.
  #
  ...
  module ClassMethods
    ...
    # default options. You can override these with +video_file_column+'s +options+ parameter
    DEFAULT_OPTIONS = {
    ...
    # in +DEFAULT_OPTIONS+.
    def video_file_column(attr, options={})
      ...
      FileColumn::Magick::video_file_column(self, attr, my_options) if options[:magick]
    end
