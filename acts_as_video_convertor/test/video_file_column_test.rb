...
class VideoFileColumnTest < Test::Unit::TestCase
  
  def setup
    # we define the video_file_columns here so that we can change
    # settings easily in a single test

    Entry.video_file_column :image
    Entry.video_file_column :file
    Movie.video_file_column :movie
  ...
  def test_sanitize_filename
    assert_equal "test.jpg", VideoFileColumn::sanitize_filename("test.jpg")
    assert VideoFileColumn::sanitize_filename("../../very_tricky/foo.bar") !~ /[\\\/]/, "slashes not removed"
    assert_equal "__foo", VideoFileColumn::sanitize_filename('`*foo')
    assert_equal "foo.txt", VideoFileColumn::sanitize_filename('c:\temp\foo.txt')
    assert_equal "_.", VideoFileColumn::sanitize_filename(".")
  end
  ...
  def test_get_content_type_with_file
    options = VideoFileColumn::ClassMethods::DEFAULT_OPTIONS.dup
    ...
    if File.executable?(options[:file_exec])
      file = VideoFileColumn::TempUploadedFile.new(options, nil, nil)
  ...
  def test_do_not_fix_file_extensions
    ...
    options = VideoFileColumn.init_options(options, "entry", "image")
    options[:fix_file_extensions] = nil

    entry = Entry.new
    file = VideoFileColumn::NoUploadedFile.new(options, entry, "image")
  ...
  def test_correct_extension
    file = VideoFileColumn::TempUploadedFile.new(VideoFileColumn::ClassMethods::DEFAULT_OPTIONS, nil, nil)
